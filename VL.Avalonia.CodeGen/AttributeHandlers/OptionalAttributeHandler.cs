...
public class OptionalAttributeHandler : IAttributeHandler
{
    ...
    public string? GenerateMethod(AttributeData attr, IFieldSymbol fieldSymbol, string fieldName)
    {
        ...
        var propertyPath =
            attr.ConstructorArguments.FirstOrDefault().Value?.ToString() ?? "<UnknownProperty>";

        var converterPath = attr.ConstructorArguments.ElementAtOrDefault(1).Value?.ToString();

        var paramBase = fieldName.TrimStart('_');
        var paramName =
            paramBase.Length > 0 ? char.ToLower(paramBase[0]) + paramBase.Substring(1) : fieldName;
        ...
        var withConverter =
            converterPath != null
                ? $"{converterPath}.ConvertToProperty({fieldName}.Value)"
                : $"{fieldName}.Value";
