...
[ProcessNode]
public abstract partial class TileBrushWrapperBase<T> : BrushWrapperBase<T> where T : TileBrush, new()
{
    ...
    public void SetDestinationRect(Optional<RectangleF> destinationRect, Optional<RelativeUnit> destinationRectRelativeUnit)
    {
        if (_destinationRect != destinationRect || _destinationRectRelativeUnit != destinationRectRelativeUnit)
        {
            if (destinationRect.HasValue)
            {
                _output.SetValue(TileBrush.DestinationRectProperty,
                    destinationRect.Value.ToRelativeRect(destinationRectRelativeUnit.HasValue ? destinationRectRelativeUnit.Value : RelativeUnit.Relative));
            }
            else
            {
    ...
    public void SetSourceRect(Optional<RectangleF> sourceRect, Optional<RelativeUnit> sourceRectRelativeUnit)
    {
        if (_sourceRect != sourceRect || _sourceRectRelativeUnit != sourceRectRelativeUnit)
        {
            if (sourceRect.HasValue)
            {
                _output.SetValue(TileBrush.SourceRectProperty,
                    sourceRect.Value.ToRelativeRect(sourceRectRelativeUnit.HasValue ? sourceRectRelativeUnit.Value : RelativeUnit.Relative));
            }
            else
            {
