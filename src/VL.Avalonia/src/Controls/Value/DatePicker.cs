...
public partial class DatePickerWrapper : ControlWrapperBase<DatePicker>
{
    ...
    {
        _selectedDateBinding = new ChannelTwoWayBinding<DateTime?, DateTimeOffset?>(_output, DatePicker.SelectedDateProperty,
    ...
    /// </param>
    [ImplementProperty("DatePicker.DayFormatProperty", PinVisibility = Model.PinVisibility.Optional)]
    protected Optional<string> _dayFormat;

    /// <param name="dayVisible">
    /// Sets if the day column is visible.
    /// </param>
    [ImplementProperty("DatePicker.DayVisibleProperty", PinVisibility = Model.PinVisibility.Optional)]
    protected Optional<bool> _dayVisible;

    /// <param name="monthFormat">
    /// Format string for the month part of the date.
    /// </param>
    [ImplementProperty("DatePicker.MonthFormatProperty", PinVisibility = Model.PinVisibility.Optional)]
    protected Optional<string> _monthFormat;

    /// <param name="monthVisible">
    /// Sets if the month column is visible.
    /// </param>
    [ImplementProperty("DatePicker.MonthVisibleProperty", PinVisibility = Model.PinVisibility.Optional)]
    protected Optional<bool> _monthVisible;

    /// <param name="yearFormat">
    /// Format string for the yar part of the date.
    /// </param>
    [ImplementProperty("DatePicker.YearFormatProperty", PinVisibility = Model.PinVisibility.Optional)]
    protected Optional<string> _yearFormat;

    /// <param name="yearVisible">
    /// Sets if the year column is visible.
    /// </param>
    [ImplementProperty("DatePicker.YearVisibleProperty", PinVisibility = Model.PinVisibility.Optional)]
    ...
    /// </param>
    [Fragment(Order = 0)]
    public void SetMinYear([Pin(Visibility = Model.PinVisibility.Optional)] Optional<DateTime> minYear)
    {
        if (_minYear != minYear)
        {
            _minYear = minYear;
            ...
            if (minYear.HasValue)
            {
                _output.SetValue(DatePicker.MinYearProperty, DateTime.SpecifyKind(minYear.Value, DateTimeKind.Local));
            }
            else
            {
    ...
    /// </param>
    [Fragment(Order = 0)]
    public void SetMaxYear([Pin(Visibility = Model.PinVisibility.Optional)] Optional<DateTime> maxYear)
    {
        if (_maxYear != maxYear)
        {
            _maxYear = maxYear;
            ...
            if (maxYear.HasValue)
            {
                _output.SetValue(DatePicker.MaxYearProperty, DateTime.SpecifyKind(maxYear.Value, DateTimeKind.Local));
            }
            else
            {
